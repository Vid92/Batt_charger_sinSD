...
ISR(RTC_OVF_vect)
{
	...
	rtc_millis = rtc_millis+3;

  //last = !last;
  //digitalWrite(27, last);
}
...
unsigned long millis(void)
{
	...
	cli();
	//unsigned long result = seconds_count*1000UL + TCD1.CNT;
	SREG = oldSREG;
  //return result + millis_count;
	return millis_count * 0.125;
}
...
uint64_t micros_huge(void) {
	...
	// inconsistent value (e.g. in the middle of a write to rtc_millis)
        ...
        // If so TCD1 count may be updated even when interrupts are off.
	uint8_t oldSREG = SREG; // Save and restore the interrupt enable bit
	cli();
	uint64_t result = ((uint64_t)seconds_count)*1000000UL + TCD1.CNT*125UL + (TCD0.CNT>>2);
	SREG = oldSREG;
...
unsigned long micros(void) {
	...
	uint8_t oldSREG = SREG; // Save and restore the interrupt enable bit
	cli();
	//unsigned long result = seconds_count*1000000UL + millis_count*125 + TCD1.CNT*125UL + (TCD0.CNT>>2);
	SREG = oldSREG;
  unsigned long result = millis_count * 125;
	return result;
}
...
{
	//unsigned long start = millis_count * 0.125;
	//while ( ( (double)(millis_count * 0.125 - start) ) <= ms);

  unsigned long start = millis();
	while ( ( (double)(millis() - start) ) <= ms);
}
...
//      EVSYS.CH0MUX  = EVSYS_CHMUX_TCD0_OVF_gc;
        ...
        TCD1.PERBUF   = 1000;
        TCD1.CTRLB    = ( TCD1.CTRLB & ~TC1_WGMODE_gm ) | TC_WGMODE_NORMAL_gc;
        TCD1.CTRLD    = TC_EVACT_UPDOWN_gc | TC1_EVDLY_bm;
...
static void initAdc( ADC_t* adc ) {
        ...
        adc->REFCTRL = AREF_VCC << ADC_REFSEL_gp //ADC_REFSEL_VCC_gc   // VCC/1.6 analog ref
